...
pub fn impl_semantic_definition_diagnostics(
    ...
    for impl_item_type_id in data.item_type_asts.keys() {
        diagnostics.extend(db.impl_type_semantic_diagnostics(*impl_item_type_id));
    }
    ...
    if let MaybeImplBody::Some(body) = impl_ast.body(syntax_db) {
        for item in body.items(syntax_db).elements(syntax_db) {
            match item {
                ...
                ImplItem::Type(ty) => {
                    let impl_type_id =
                        db.intern_impl_type(ImplTypeDefLongId(module_file_id, ty.stable_ptr()));
...
pub fn priv_impl_type_semantic_data(
    ...
    let mut diagnostics = SemanticDiagnostics::new(module_file_id.file_id(db.upcast())?);
    let impl_types = db.impl_types(impl_type_id.impl_def_id(db.upcast()))?;
    let impl_type_ast = impl_types.get(&impl_type_id).to_maybe()?;
    let generic_params_data = db.priv_impl_type_generic_params_data(impl_type_id)?;
...
fn validate_impl_function_signature(
    ...
    {
        let expected_ty = reduce_impl_type_if_possible(db, trait_param.ty, Some(impl_def_id))?;
        let actual_ty = reduce_impl_type_if_possible(db, param.ty, Some(impl_def_id))?;
    ...
    let expected_ty =
        reduce_impl_type_if_possible(db, concrete_trait_signature.return_type, Some(impl_def_id))?;
    let actual_ty = reduce_impl_type_if_possible(db, signature.return_type, Some(impl_def_id))?;
    if expected_ty != actual_ty {
